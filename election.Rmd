...
contributions.zipcodes <- inner_join(contributions.zipcodes, zipcodes)

ggplot() + 
  geom_polygon(data = usa, aes(x = long, y = lat, group = group), fill=NA, color="black") + 
  geom_polygon(data = states, aes(x = long, y = lat, fill = region, group = group)) + 
  coord_fixed(1.3) +
  geom_point(data = contributions.zipcodes, aes(x = lon, y = lat)) +
  guides(fill = FALSE)

```

At first, I assumed that all the donations would be from California, but it turns out
that donations came from various parts of the United States.
...
contributions.candidates <- contributions %>% 
  ...
  summarise(total = sum(contb_receipt_amt, na.rm = TRUE),
            ...
            party = first(party)) %>%
  arrange(-n) %>%
  mutate(cand_nm = factor(cand_last_name, cand_last_name)) 
...
contributions$cand_nm = factor(contributions$cand_nm, levels=rev(cand.order), ordered=TRUE)
